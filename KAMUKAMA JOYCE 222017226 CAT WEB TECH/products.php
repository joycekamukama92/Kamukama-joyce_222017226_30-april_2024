...
<h1>Products Form</h1>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO products (product_id, product_name, description,category,price) VALUES (?,?,?,?,?)");
        $stmt->bind_param("issss", $product_id, $product_name, $description,$category,$price);

        // Set parameters from POST data with validation (optional)
        $product_id = ($_POST['emp_Id']); // Ensure integer for ID
        $product_name = ($_POST['dpid']); 
        $description = ($_POST['fnm']); 
        $category = ($_POST['lnm']); 
        $price = ($_POST['prc']); 
...
<body>
    ...
    <table border="5">
        ...
        // Prepare SQL query to retrieve all product
        $sql = "SELECT * FROM products";
        $result = $connection->query($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $emid = $row['product_id']; // Fetch the product_id
                echo "<tr>
                    ...
                    <td>" . $row['category'] . "</td>
                    <td>" . $row['price'] . "</td>
                    <td><a style='padding:4px' href='delete_product.php?product_id=$emid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_product.php?product_id=$emid'>Update</a></td> 
                </tr>";
            }
        } else {
